...
`spark_map` is a python package that contains an implementation of `map()` function for Apache Spark DataFrames, using `pyspark`. In other wrods, the `spark_map` package offers tools to apply a function over multiple columns of a Spark DataFrame. The package offers two main functions to do that, which are `spark_map()` and `spark_across`.

Both of these functions perform the same work, which is to apply a function over a set of columns of a Spark DataFrame. But the difference between them, is that `spark_map()` uses the `agg()` method of Spark DataFrame's to apply the function, while `spark_across()` uses the `withColumn()` method to do so.
...
The full documentation for `spark_map` package is available in this website. To access it, just use the `Function Reference` and `Articles` menus, located at the top navigation bar of this page. All documentation is available both in [english](reference-en.qmd) and in [portuguese](reference-ptbr.qmd).
